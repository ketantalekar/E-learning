...
%>
  ...
  String query2="Select Subject_name,Subject_code,Department from subject_details where Userid=?";  
  ...
  while(rs5.next())
  {
	  subject=rs5.getString(1);
	 subject_code=rs5.getString(2);
	  dept=rs5.getString(3);
	  %>
	  
	<div name="subject"class="col-xl-3 col-md-6 mb-4">
              <div class="card border-left-success shadow h-100 py-2">
                <div onclick="window.location.replace('test.jsp?subjectcode=<%=subject_code%>');"class="card-body">
  ...
  <span style="color:black;"><%out.println(subject); %></span>
   </div>
     <div class="deptname">
                   <a style="color:black;text-decoration:none;" name="studsubject" href="#" ><%=dept%></a>
   </div>
                  </div>
                </div>
                <div class="card-footer subcode">
                <input type="text" class="copytext"value="<%=subject_code%>" id="<%=subject_code %>" title="subject code" readonly>
                <button name="btncopy"style="border: none;background: none;font-size:20px;" onclick="myFunction('<%=subject_code %>')" title="copy to clipboard">
