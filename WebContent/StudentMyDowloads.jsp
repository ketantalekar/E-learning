      ...
      <ul class="navbar-nav ml-auto">
       <form class="navbar-search mt-3 float-right">
            <div class="input-group">
              <input type="text" class="form-control bg-silver border-1 small" id="myInput" onkeyup="myFunction()" autocomplete="off"placeholder="Search for Filename" aria-label="Search" aria-describedby="basic-addon2">
...
%>
  ...
  <li class="nav-item dropdown no-arrow mx-1">
   ...
   <div id="file">
   <h3 style="color:black;margin-left:50px;"><i class="fa fa-download" aria-hidden="true"></i>&nbsp;Downloads</h3>
     <%   
  int cnt=0,flag=0;
  String sql = "Select Id,Userid,File_Name,Download_date from downloads where Userid=? order by Id desc ";
  PreparedStatement pstm = con1.prepareStatement(sql);
  pstm.setString(1,user);
  ResultSet rs = pstm.executeQuery();
  %>
  <table cellspacing="6" cellpadding="25"id="myTable">
  <tr><th>Id</th><th>File Name</th><th>Downloaded On</th><th>Size</th></tr>
  ...
  String download=rs.getString("Download_date");
  String sql3 = "Select File_data from upload_file where File_Name=? union all Select File_data from facultyassignment where File_Name=? ";
  PreparedStatement pstm3 = con1.prepareStatement(sql3);
  pstm3.setString(1,fileName);
  pstm3.setString(2,fileName);
  
  ResultSet rs3 = pstm3.executeQuery();
  if(rs3.next())
  {
	  fdata=rs3.getBlob("File_data");
...
<td><%=cnt %></td>
<td id="fname"><a href="/E-learning/FacultyViewFile?filename=<%=fileName%>" target="_blank" style="text-decoration:none;"><%=fileName.substring(0, Math.min(fileName.length(), 40)) %></a></td>
<td><%=download %></td>
<td><%
double bytes=fdata.length();
double kb=(bytes/1024);
double size=Math.round(kb*100.0)/100.0;
out.println(size+" kb");
%>
</td>
<% 
