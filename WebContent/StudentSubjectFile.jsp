...
%>  
<div style="margin-left:60px;color:black;margin-top:10px;position:absolute;"><h3><i class="fas fa-chalkboard-teacher" aria-hidden="true"></i>&nbsp;Files</h3></div>
  
   <div id="file">

   
   <form method="post" id="f1" action="/E-learning/FacultyUploadFile"enctype="multipart/form-data">
  <div id="uploadfile">
...
document.querySelector("#file-input").onchange = function(){
     ...
     <%   
  int cnt=0,flag=0;
  String sql = "Select Id,Userid,File_Name,File_data,Upload_date from upload_file where Subject_Name=? order by Id desc ";
  PreparedStatement pstm = con1.prepareStatement(sql);
  pstm.setString(1,subject);
  ResultSet rs = pstm.executeQuery();
  %>
  <table cellspacing="6" cellpadding="25">
  <tr id="pp"><th>Id</th><th>Name</th><th>Modified Date</th><th>Modified by</th><th>Download</th><th>Remove</th></tr>
  
  <% 
  PreparedStatement pstmt44=null;
  String facname2="";
  while(rs.next()) 
  {
	  cnt++;
  ...
  String userid=rs.getString("Userid");
	...
	pstmt44=con1.prepareStatement("SELECT Username FROM faculty_details WHERE Userid=?");
	pstmt44.setString(1,userid);
	ResultSet rs44=pstmt44.executeQuery();
	
if(rs44.next())
	{
		facname2=rs44.getString(1);
	
}
  //out.println(fileData.length());
  String q2="Select Username from faculty_details where Userid=? ";
  PreparedStatement pstm1 = con1.prepareStatement(q2);
  ...
  else
  {
	  ...
	  flag=0;
	  String q3="Select Username from student_details where Userid=? ";
	  PreparedStatement pstm2 = con1.prepareStatement(q3);
	  pstm2.setString(1,userid);
	  ResultSet rs3 = pstm2.executeQuery();  
	  if(rs3.next())
	  {
	 uname=rs3.getString(1);
	  }
  }
...
<td><%=cnt %></td>
<td id="fname"><a href="/E-learning/FacultyViewFile?filename=<%=fileName%>" target="_blank" style="text-decoration:none;"><%=fileName.substring(0, Math.min(fileName.length(), 40)) %></a></td><td><%=upload_date %></td>
<td><%=uname %></td>
<td id="tableicon"><a href="FacultyFileDownload.jsp?id=<%=id%>"><i class="fa fa-download" style="color:grey;font-size:25px;"aria-hidden="true"></i></a></td>
<td id="tableicon"><% 
String uid=(String)session.getAttribute("sessuid");
if(userid.equals(uid)){
%>	
	<a href=""data-toggle="modal" data-target="#D<%=id%>"><i class="fa fa-trash" style="color:grey;font-size:25px;"aria-hidden="true"></i></a></td>
<%
}
else
{
%>
<i style="color:black;"class="fa fa-minus" aria-hidden="true"></i>
   ...
   <div class="modal-dialog modal-confirm" role="document"> 
     ...
     <!-- login modal content -->
      ...
      <div class="modal-footer justify-content-center">
				...
				<a href="FacultyFileDelete.jsp?id=<%=id%>" style="color:white;padding-top:10px;"class="btn btn-danger">Delete</a>
			</div>
