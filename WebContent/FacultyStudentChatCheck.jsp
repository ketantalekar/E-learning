...
if(request.getParameter("people")!=null)	
{
	...
	String bs1="select Userid from student_details where Username=?";
	...
	String bs="select * from student_details where Userid='"+replaceString+"' and  Userid <>'"+me+"'";
	st.execute(bs);
	...
	flag++;
	 ...
	 if(flag==1)
	 {
		session.setAttribute("searchuname",userid);
			response.sendRedirect("FacultyStudentChat.jsp?uname="+userid);

	 }
	 else
	 {
		 String name="User not found";
			response.sendRedirect("FacultyStudentChat.jsp?unam="+name);
