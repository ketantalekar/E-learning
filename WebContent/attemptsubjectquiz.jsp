...
%>
<div id="atquiztitle">Title:<%=title%>
</div>
<hr id="line">
<div id="note">Description:</div>
<div id="details"><%=description%></div>
<hr id="line">
<div id="note">Total Marks:</div>
<%
...
{
	%>
<div id="details"><%=rs8.getInt(1)  %></div>
<%	
}
...
String subtime="";
int qqid=0;
String sql12 = "Select Submitted_time,Id from quizresult where title=? and Subject_code=? and Userid=?";
  PreparedStatement pstm12 = con1.prepareStatement(sql12);
  pstm12.setString(1,title);
  pstm12.setString(2,subject);
 pstm12.setString(3,(String)session.getAttribute("sessuid"));
  ...
  String preview="";
  String sql14 = "Select Preview from quiz_info where title=? and subject=?";
  PreparedStatement pstm14 = con1.prepareStatement(sql14);
  pstm14.setString(1,title);
  pstm14.setString(2,subject);
  ResultSet rs14 = pstm14.executeQuery();
  if(rs14.next())
  {
	preview=rs14.getString(1); 
  }
...
String questionssss="";
int cnt1=0;
String sql13 = "Select Question from quiz_questions where title=? and Subject_code=?";
  PreparedStatement pstm13 = con1.prepareStatement(sql13);
  pstm13.setString(1,title);
  pstm13.setString(2,subject);
  ResultSet rs13 = pstm13.executeQuery();
  while(rs13.next())
  {
	  cnt1++;
	  questionssss=rs13.getString(1); 
...
<hr id="line">
<div id="note">Created at:</div>
<div id="details"><%=created_date %></div>
<hr id="line">
<div id="note">Start Date:</div>
<div id="details"> <%=quiz_date %></div>
<hr id="line">
<div id="note">Start Time</div>
<div id="details"><%=quiz_time%></div>
<hr id="line">
<div id="note">Time Limit:</div>
<div id="details">
<% if(s[0].equals("0"))
{ 
	out.println(s[2]+" "+s[3]); 
...
cal1.setTime(date1);
	...
	String ndate=sdf.format(cal1.getTime());
//out.println(newTime);
Date date6 = parseFormat.parse(newTime);
String dtime1=displayFormat.format(date6);
//out.println(dtime1);
...
if(cnt1>0)
{
if(subtime==null || subtime.equals(" ") || subtime.length()==0)
{                              //if subtime
if((quiz_date.compareTo(ndate)>=0 && dateString.compareTo(dtime)<=0) || (quiz_date.compareTo(d1)==0 && dateString.compareTo(dtime)>=0))
{                               //if date compare
if(dateString.compareTo(dtime1)<0)	
{                                //if date2 compare
%>
		
	<a href="attemptsubjectquizdb.jsp?id=<%=qid%>&page=1&cnt=0"><input type="submit" value="Attempt Now" name="submit" id="btn-quiz"></a>
...
{                              //else date2
	%>
	<span style="color:red"><center>This Quiz is finished</center></span>
...
{                                  //else date                           
	%>
	<span style="color:red"><center>This Quiz is not available right now</center></span>
...
<% 
  ...
  {
	  %>
	  <span style="color:red"><center>Already Submitted this quiz</center></span>
	  <%
	  if(preview.equals("Yes"))
	  {
	  %>
	  <div style="margin-top:10px;"></div>
	 <center> <a style="text-decoration:none;color:white;"href="studentviewquizmarks.jsp?id=<%=qid%>"><button style="background:#2471A3;color:white;outline:none;border:0px;"type="submit"><i class="fas fa-eye"></i> Preview</button></a></center>
  ...
  {
	  %>
	  <span style="color:red"><center>Exam is not available</center></span>
  <% 
  }
}
    }
