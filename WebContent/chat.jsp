...
if(request.getParameter("people")!=null)	
{
	...
	String userid="";
	 PreparedStatement ps7=con.prepareStatement("Select Userid from college_admin_details where Username=?");
	...
	String bs="select * from college_admin_details where Userid='"+replaceString+"' and  Userid <>'"+me+"'";
	st.execute(bs);
	...
	flag++;
	 ...
	 {		 int pstatus=0;
		...
		session.setAttribute("searchuname",userid);
			response.sendRedirect("Vendorchat.jsp?uname="+userid);

	 }
	 else
	 {
		 String name="User not found";
			response.sendRedirect("Vendorchat.jsp?unam="+name);
