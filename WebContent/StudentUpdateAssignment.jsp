...
//Integer a=(Integer)session.getAttribute("sessassignid");
...
String assigntitle="";
String description="";
String duedate="",date="",time="",duetime="",filename="";
String filename1="",submit_date="",submit_time="",assigntitle1="";
String q1="Select id,Assignment_name,Assignment_file,File_data,Submitted_date,Submitted_time from student_assignments where id=?";
PreparedStatement p1=con1.prepareStatement(q1);
p1.setInt(1,id);
ResultSet rs2=p1.executeQuery();
if(rs2.next())
{
	id1=rs2.getInt(1);	
	assigntitle1=rs2.getString(2);
	filename1=rs2.getString(3);
	submit_date=rs2.getString(5);
	submit_time=rs2.getString(6);
	
	String q2="Select Id,Title,File_name,File_data,Date,Time,DueDate,DueTime,Description from facultyassignment where Title=?";
	PreparedStatement p2=con1.prepareStatement(q2);
	p2.setString(1,assigntitle1);
	ResultSet rs3=p2.executeQuery();
	if(rs3.next())
	{
		id=rs3.getInt(1);	
		assigntitle=rs3.getString(2);
		filename=rs3.getString(3);
		date=rs3.getString(5);
		time=rs3.getString(6);
		duedate=rs3.getString(7);
		duetime=rs3.getString(8);
		description=rs3.getString(9);
		
	}

	
}
out.println("<div id='assigntitle'>"+assigntitle+"</div>");
out.println("<div id='duedate'>Due "+duedate);
out.println(duetime+"</div>");

out.println("<div id='note'>Note:</div>");
out.println("<span style='margin-top:5px;'>"+description+"</span>");
out.println("<div id='note'>Material: </div>");
%>
<a href="StudentAssignmentDownload.jsp?id=<%=id%>" style="color:black;">
<span style='margin-top:5px;'><%=filename%></span></a>
<% 
//out.println("<span style='margin-top:5px;'>"+filename+"</span>");

out.println("<div id='duedate'>Submitted On "+submit_date);
out.println(submit_time+"</div>");

%>
<%
  
  out.println("<div id='note'>Submitted File: </div>");
%>
<a href="SubmittedAssignmentDownload.jsp?id=<%=id1%>" style="color:black;">
<span style='margin-top:5px;'><%=filename1%></span></a>
<% 
 // out.println("<span style='margin-top:5px;'>"+filename1+"</span>");

  %>

 <div id="note">     
    <form method="post" id="f1" action="/E-learning/StudentUpdateAssignment"enctype="multipart/form-data">
  <div id="uploadfile">
   <label for="file-input"title="upload file">
   <i class="fa fa-paperclip" aria-hidden="true" style="color:blue;font-size:30px;"></i> &nbsp;Attach work
      
  </label>
  <input type="hidden" name="assigntitle" value="<%=assigntitle1%>">
<input type="hidden" name="assignid" value="<%=id1%>">

    <input id="file-input" name="file" type="file" style="display:none;"/>
      <label id="file-name"></label> 
   <!--   <input type="submit" id="b1" > -->
          </div>
          <input style="display:inline-block;"type="submit" name="submit" value="Update Assignment">
          
    </form>
    <a style="display:inline-block;margin-left:200px;margin-top:-35px;position:absolute;"href="UndoSubmitAssignment.jsp?id=<%=id1%>"><input type="button" value="Undo Submit"></a>
    <%
    }
catch(Exception e)
{
