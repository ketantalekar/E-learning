	   ...
	   if(sen.equals(sender))
		  {
		   String recname="";
		      ...
		      recname=rs5.getString("Username");
		   		   ...
		   		   %>
		   		   <a onclick="openNav1()"id="recent-chats-link"href="AllGroupChatCheck.jsp?groupname=<%=grpnm%>"><div id="recent-chats1">
...
<div id="showList2">
  ...
  else
  {
	  ...
	  ResultSet res3=pst.executeQuery();
	  while(res3.next()){
		  String user2=res3.getString(1);
		  String uid=res3.getString(2);
		  %>
		  <a id="recent-chats-link"href="AllChatCheck.jsp?people=<%=user2%>">
		  <img class="img-chat rounded-circle" width="95px" height="50px" src="AllViewImage.jsp?name=<%=uid%>">
	  
		  <%
	      out.println("<div style='display:inline-block;color:black'>"+user2.substring(0, 1).toUpperCase() +user2.substring(1)+"</div><hr id='chat-line'></a>");
	  }
    	...
    	<%
    	 ...
    	 else
    	 {
    		...
    		<%  
                ...
                <button type="button" onclick="window.location.replace('Deletegroup.jsp');"name="delgrp"class="btn btn-danger btn-block btn-lg"><i class="fa fa-trash" aria-hidden="true"></i>  Delete Group</button>
                <%
                }
                else
                {
...
function closeNav() {
  ...
  {
	  window.location.replace("CollegeAdminchats.jsp");
