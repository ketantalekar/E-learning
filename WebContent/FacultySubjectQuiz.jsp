...
%>
  <div style="margin-left:60px;color:black;margin-top:-2px;position:absolute;"><h3><i class="fas fa-puzzle-piece" aria-hidden="true"></i>&nbsp;Quiz</h3></div>
  
   <div id="test">
   <a href="" data-toggle="modal" data-target="#addquiz" id="addnew"><i class="fa fa-plus" aria-hidden="true"></i> Create a quiz </a>
  
<div id="addquiz" class="modal fade" tabindex="-1" role="dialog">
   <div class="modal-dialog" role="document"> 
    
     <!-- login modal content -->
        <div class="modal-content" id="login-modal-content">        
            <div class="modal-header text-center">
        <h4 class="modal-title w-100 font-weight-bold"> Create a quiz</h4>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
    </div>
          <div class="modal-body mx-3">
          <form action="/E-learning/FacultyAddQuiz" method="POST">
    <div class="form-group">
     
                <div class="input-group">
                <div class="input-group-prepend"><span class="input-group-text"><i class="fa fa-info-circle" aria-hidden="true"></i></span></div>
            	<input name="quiztitle" id="quiztitle" autocomplete="off"type="text" class="form-control input-lg" placeholder="Enter Quiz Title" required>
                	</div>
      </div>   
            <div class="form-group">
                
   <textarea name="description" placeholder="Description about Quiz"></textarea>
                 </div>        
                 <label for="date">Set Quiz Date </label>
 
                <input style="outline:none;"id="date"type="date"name="quizdate">
<div class="md-form md-outline">
 <label for="default-picker">Set Quiz Time </label>
  <input type="time" name="quiztime" id="default-picker" class="form-control" placeholder="Select time">
...
<div class="form-group">
            	...
            	<input name="timelimit1"id="timelimit" autocomplete="off"type="number"class="form-control input-lg" placeholder="Time(Hrs)">
                	<select style="border:1px solid lightgrey;outline:none;"name="hr">
                	<option value="hrs">hrs</option>
                	</select>
                	<input name="timelimit2"id="timelimit" autocomplete="off"type="number"class="form-control input-lg" placeholder="Time(Mins)">
                	<select style="border:1px solid lightgrey;outline:none;"name="min">
      ...
      <div class="form-group">
  <label for="sel1">Marks Preview</label>
  <select class="form-control" name="preview"id="sel1">
    <option>Yes</option>
    <option>No</option>
   </select>
</div>
<br>
       <button type="submit" class="btn btn-primary btn-block btn-lg"><i class="fa fa-plus" aria-hidden="true"></i> Create Quiz</button>
  </div>
  ...
  try{
  int cnt=0;
  String sql = "Select Id,title,created_date,quiz_date,quiz_time,time_limit,description,Preview from quiz_info where Faculty_name=? and Subject=? order by id desc";
  ...
  <tr id="mmm"><th>#</th><th>Title</th><th>Description</th><th>Created date</th><th>Quiz date</th><th>Quiz time</th><th>Time limit</th><th>Action</th></tr>
  <tr>
  <% 
  ...
  String quiz_time = rs.getString("quiz_time");
  String quiz_date=rs.getString("quiz_date");
  String time_limit=rs.getString("time_limit");
  //out.println(time_limit);
  String description = rs.getString("description");
  String preview=rs.getString("preview");
  String[] sa=quiz_date.split("/");
  String disdate=sa[2]+"-"+sa[1]+"-"+sa[0];
  SimpleDateFormat displayFormat = new SimpleDateFormat("HH:mm");
  SimpleDateFormat parseFormat = new SimpleDateFormat("hh:mm aa");
  Date date = parseFormat.parse(quiz_time);
  ...
  //out.println(tlimit);
  //out.println(s[0]); s[0]=1 s[1]=hrs s[2]=30 s[3]=mins
  ...
  double st1=Double.parseDouble(s[0]);
  double st3=st1*60;
int st2=(int)st3;
//out.println(st2);
...
// out.println(quiz_date);
 //out.println(d1);
 //out.println(dateString);
 //out.println(quiz_time);
 //out.println(quiz_date.compareTo(d1));
 //out.println(dateString.compareTo(quiz_time));
 //out.println(newTime);
 String[] s1=newTime.split(" ");
 String t=s1[1];
//out.println(t);
//out.println(quiz_time);
//out.println(dateString);
//out.println(dtime);
 //out.println((quiz_date.compareTo(d1)>0 && dateString.compareTo(dtime)>=0) || (quiz_date.compareTo(d1)==0 && dateString.compareTo(dtime)<=0));
 //out.println(dateString);
 ...
 //out.println(dateString);
 //out.println(dtime);
  if((quiz_date.compareTo(d1)>0) || (quiz_date.compareTo(d1)==0 && dateString.compareTo(dtime)<=0))
  {                               //if date compare
  %>
  <td onclick="location.href='addquizquestions.jsp?id='+<%=id%>;"><%=cnt %></td>
<td onclick="location.href='addquizquestions.jsp?id='+<%=id%>;"><%=title%></td>
<td id="tp" onclick="location.href='addquizquestions.jsp?id='+<%=id%>;"><%=description%></td>
<td onclick="location.href='addquizquestions.jsp?id='+<%=id%>;"><%=created_date %></td>
<td onclick="location.href='addquizquestions.jsp?id='+<%=id%>;"><%=quiz_date %></td>
<td onclick="location.href='addquizquestions.jsp?id='+<%=id%>;"><%=quiz_time%></td>
<td onclick="location.href='addquizquestions.jsp?id='+<%=id%>;">
<% if(s[0].equals("0"))
{ 
	out.println(s[2]+" "+s[3]); 
	}
...
<%
  ...
  {
  %>
 <td onclick="location.href='QuizResponse.jsp?id='+<%=id%>;"><%=cnt %></td>
<td onclick="location.href='QuizResponse.jsp?id='+<%=id%>;"><%=title%></td>
<td id="tp"onclick="location.href='QuizResponse.jsp?id='+<%=id%>;"><%=description%></td>
<td onclick="location.href='QuizResponse.jsp?id='+<%=id%>;"><%=created_date %></td>
<td onclick="location.href='QuizResponse.jsp?id='+<%=id%>;"><%=quiz_date %></td>
<td onclick="location.href='QuizResponse.jsp?id='+<%=id%>;"><%=quiz_time%></td>
<td onclick="location.href='QuizResponse.jsp?id='+<%=id%>;">
<% if(s[0].equals("0"))
{ 
	out.println(s[2]+" "+s[3]); 
	}
...
<td>
<a class="btn btn-md btn-primary" href="#E<%=id %>" data-toggle="modal"><i class="fa fa-edit" aria-hidden="true"></i> Edit</a><span style="width:10px"></span>
...
<div id="E<%=id %>" class="modal fade" tabindex="-1" role="dialog">
   <div class="modal-dialog" role="document"> 
     ...
     <!-- login modal content -->
        <div class="modal-content" id="login-modal-content">        
            <div class="modal-header text-center">
        <h4 class="modal-title w-100 font-weight-bold">  Edit quiz</h4>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
    </div>
          <div class="modal-body mx-3">
          <form action="/E-learning/FacultyEditQuiz" method="POST">
               <div class="form-group">
            <label for="quiztitle">Quiz Title:</label>
         	<input name="quiztitle" id="quiztitle" autocomplete="off"type="text" class="form-control input-lg" value="<%=title %>" required>
                	</div>
      
            <div class="form-group">
                <label for="description">Description:</label>
   <textarea style="  padding: 2px 2px;"name="description"required><%=description %></textarea>
                 </div>        
                 <div style="margin-top:-10px;"><label for="date">Set Quiz Date </label>
  <input style="margin-top:-5px;outline:none;"id="date2"type="date"name="quizdate" value="<%=disdate%>"required></div>
                
<div class="md-form md-outline">
 <label for="default-picker">Set Quiz Time </label>
  <input type="time" name="quiztime" id="default-picker" class="form-control" value="<%=dtime%>"required>
...
<div style="height:5px;"></div>
     <label for="timelimit">Time Limit </label>
                <div class="input-group">
                <div class="input-group-prepend"><span class="input-group-text"><i class="fa fa-clock-o" aria-hidden="true"></i></span></div>
            	<input name="timelimit1"id="timelimit" autocomplete="off"type="number"class="form-control input-lg" value="<%=s[0]%>">
                	<select style="border:1px solid lightgrey;outline:none;"name="hr">
                	<option value="hrs">hrs</option>
                	</select>
                	<input name="timelimit2"id="timelimit" autocomplete="off"type="number"class="form-control input-lg" value="<%=s[2]%>">
                	<select style="border:1px solid lightgrey;outline:none;"name="min">
                	<option value="mins">mins</option>
                	</select>
  ...
  <select class="form-control" name="preview"id="sel1">
  	<%
                	...
                	{
                		%>
                		<option value="<%=preview%>">Yes</option>
                	...
                	{
                		%>
                		<option value="<%=preview%>">No</option>
...
<input type="hidden" name="qname" value="<%=title %>">
       <button type="submit" class="btn btn-primary btn-block btn-lg"><i class="fa fa-edit" aria-hidden="true"></i> Modify</button>
         </form>
   ...
   <div class="modal-dialog modal-confirm" role="document"> 
     ...
     <!-- login modal content -->
        <div class="modal-content" id="login-modal-content">        
          ...
          <p>Do you really want to delete this Quiz? This process cannot be undone.</p>
      </div>
      <div class="modal-footer justify-content-center">
				<button type="button" class="btn btn-secondary" data-dismiss="modal">Cancel</button>
				<a href="DeleteQuiz.jsp?id=<%=id%>" style="color:white;padding-top:10px;"class="btn btn-danger">Delete</a>
			</div>
